...
"""
class ExtremelyRandomDecisionTree:
	def __init__(self, maximum_depth=10**10, min_examples_per_leaf=1):
		...
		self.attrs	= AttributeSelector("extremely random").get_attribute_selector()
...
def test_ERDT_1():
	...
	X[:,-1] = X[:,-1] % classes
	Y = Y % classes
	clf = ExtremelyRandomDecisionTree()
	clf.fit(X,Y)
...
def test_ERDT_2():
	...
	X = np.arange(rows*cols).reshape(rows,cols)
	Y = np.arange(rows)
	clf = ExtremelyRandomDecisionTree(maximum_depth=40, min_examples_per_leaf=1)
	clf.fit(X,Y)
	example = np.arange(cols)+12*cols
...
def test_ERDT_3():
	...
	clf = ExtremelyRandomDecisionTree(maximum_depth=10, min_examples_per_leaf=1)
	clf.fit(X,Y)
	counter = 0
